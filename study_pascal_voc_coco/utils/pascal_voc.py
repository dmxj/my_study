...
if __name__ == '__main__':
    ...
    from utils import data_anna_load
    import cv2

    img_path = "../data/2007_000032.png"
    anno_path = "../data/2007_000032.xml"

    data_anno = data_anna_load.load_pascal_single(anno_path,"../data")
    ...
    print("bbox 0 is:",bbox_list[0])
    mask = getsegmentation(img_path,bbox_list[0])
    print("mask of bbox 0 is:",mask)

    print()
    bbox = mask2box(mask)
    print("bbox recover from mask is:",bbox)

    print()
    polygons = mask2polygons(mask)
    print("get polygons from mask is:",polygons)

    x_points = [p[0] for p in polygons]
    y_points = [p[1] for p in polygons]
